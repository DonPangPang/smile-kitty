...
public class AvatarResourceController(IUnitOfWork<AvatarResource> unitOfWork, IWebHostEnvironment webHostEnvironment)
    : ApiControllerBase
{
    private readonly IUnitOfWork<AvatarResource> _unitOfWork = unitOfWork;
    ...
    public async Task<IActionResult> UploadFile(IFormFile? file, [FromQuery] int width = 512, [FromQuery] int height = 512)
    {
        ...
        var resource = new AvatarResource()
        {
            Name = file.FileName,
            Extension = extension,
            Url = resourceUri,
